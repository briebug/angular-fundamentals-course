...
export const checkListItemHas = (cypIndex, value) =>
  cy.get(`[data-cy=item-title]:${cypIndex}`).should('contain.text', value);
...
export const fillForm = (info) =>
  getForm().within(($form) =>
    cy
      .get('[data-cy=input]')
      .each((element) => cy.wrap(element).type(`${info}`))
  );
...
export const checkFormEmpty = () =>
  getForm().within(($form) =>
    cy
      .get('[data-cy=input]')
      .each((element) => cy.wrap(element).should('be.empty'))
  );

export const checkFormInputHas = (value) =>
  getForm().within(($form) =>
    cy.get('[data-cy=input]:last').should('have.value', value)
  );
